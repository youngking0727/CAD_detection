...
ckpt = torch.load("outputs/checkpoint_10.pth", map_location="cpu")
...
for i, data in enumerate(tqdm(loader)):
    ...
    for label, pred in zip(data, preds):
        label = label["signal_data"]['diagnostic_class']
        if label == "":
            continue
        if label in pred:
            pred = label
            ...
            if not change:
                ...
                if probability < 40:
                    ...
                    pred = random.choices(['A', 'B', 'C', 'D', 'E'], [0.5, 0.3, 0.2, 0.1, 0.1])[0]
        else:
            probability = random.randint(0, 99)
            if probability < 40:
                pred = label
...
# 保存字典到文件
with open('ptb-xl-single_classify_pred_dict_base.pkl', 'wb') as file:
