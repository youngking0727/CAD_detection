...
def add_arguments(parser):
    ...
    parser.add_argument("--save_path", type=str, default="./outputs3/")
    parser.add_argument("--save_train_stats", action="store_true")
    parser.add_argument("--log_path", type=str, default="./logs/")
    parser.add_argument("--resume", type=bool, default=False)
    parser.add_argument("--resume_checkpoint", type=str, default="./outputs2/checkpoint_8.pth")
...
def train(loader, model, optimizer, schedular, epoch, config, device):
    ...
    for i, data in enumerate(logger.log_every(loader, 1000, header)):
        ...
        if not args.debug and (i + 1) % 5000 == 0:
            if is_main_process():
                save_dict = {
                    "epoch": epoch,
