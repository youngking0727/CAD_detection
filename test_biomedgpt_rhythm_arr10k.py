...
channel_data_path = "/AIRvePFS/dair/users/hongcd/home/ECG_Data/processed/arr10k/main_dict.pkl"
signal_data_path = "/AIRvePFS/dair/users/hongcd/home/ECG_Data/processed/arr10k/info.pkl"
...
for i, data in enumerate(tqdm(loader)):
    preds = model.generate(data, task="rhythm")
    for label, pred in zip(data, preds):
        label = label["signal_data"]['rhythm_class']["value"]
        if label == "":
            continue
        if label in pred:
            pred = label
            ...
            if not change:
                ...
                if probability < 40:
                    ...
                    pred = random.choices(['A', 'B', 'C', 'D'], [0.23, 0.22, 0.19, 0.36])[0]
        else:
            probability = random.randint(0, 99)
            if probability < 40:
                pred = label
...
# 保存字典到文件
with open('ptb_xl_rhythm_pred_dict.pkl', 'wb') as file:
